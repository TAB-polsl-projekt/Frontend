...
define_api_response!(pub enum Response {
    Ok => (200, "Returns user id and solution data", Vec<(String, Solution)>, ()),
});
...
#[get("/subject/<subject_id>/student-assignments")]
pub async fn endpoint(subject_id: String, conn: crate::db::DbConn, session: Session) -> Result<Response, Error> {
    ...
    let result = conn.run(move |c| {
        subjects::table
            ...
            .select((user_solution_assignments::user_id, solution::all_columns))
            .get_results(c)
    }).await?;
